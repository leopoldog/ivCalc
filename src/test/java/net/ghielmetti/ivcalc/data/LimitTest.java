    ...
    for (int s = 1; s <= 7; s++) {
      for (char i = '1'; i <= '4'; i++) {
        ...
        for (char l = 'a'; l <= 'd'; l++) {
          ...
          missing = missing < 0 ? 0 : missing > 15 ? 15 : missing;
          IVLevel iv = new IVLevel((s & 2) == 0 ? missing : val, (s & 1) == 0 ? missing : val, (s & 4) == 0 ? missing : val);
          if (missing >= 0 && missing < val && iv.getTotal() >= minSum && iv.getTotal() <= maxSum) {
            assertTrue(description + " : " + iv + " must match " + limit, limit.matches(iv));
          } else {
            assertFalse(description + " : " + iv + " mustn't match " + limit, limit.matches(iv));
          }
        }
      }
    }
